...
public class followzobi : MonoBehaviour {
	...
	void Update () {
		Debug.DrawLine (player.transform.position, zobi.transform.position, Color.green);
		if (follow == true) {
			nav.Resume ();
			zobi.GetComponent<Animation> ().Play ("zombie");
			...
			nav.Stop ();
			zobi.GetComponent<Animation> ().Play("azombie");

		} else {
			nav.Stop();
			zobi.GetComponent<Animation> ().Play("lzombie");
		}
